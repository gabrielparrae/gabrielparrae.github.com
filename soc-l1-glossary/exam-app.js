...
function prepareExamQuestions(questions) {
    return shuffleArray(questions).map(q => {
        // Normalizar el índice de respuesta correcta para preguntas con
        // `correct` o `correctIndex`.
        const originalCorrectIndex = Number.isInteger(q.correctIndex)
            ? q.correctIndex
        ...
        // Mezclar opciones sin mutar el arreglo original
        const shuffledOptions = shuffleArray([...q.options]);
        const newCorrectIndex = Math.max(shuffledOptions.indexOf(correctOptionText), 0);

        return {
            ...q,
            options: shuffledOptions,
            correctIndex: newCorrectIndex,
            correct: originalCorrectIndex,
